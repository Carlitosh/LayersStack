...
def CreateGridPointFromPolygon(inputfile_polygon, outfile_point, cellsize = 10):
    """
    inputfile_polygon = "./AGD-Canals_Duralde_1_boundary_14.shp"   
    outfile_point = "./salida2.shp"
    CreateGridPointFromPolygon(inputfile_polygon, outfile_point)
    
    """
    df = gp.read_file(inputfile_polygon)
    df.to_crs({'init': 'epsg:32720'})
    xmin, ymin, xmax, ymax = df.total_bounds
    xmin = xmin-50
    xmax = xmax+50
    ymin = ymin-50
    ymax = ymax+50
    xx, yy = np.meshgrid(np.arange(xmin + cellsize/2, 
                                   xmax + cellsize/2, cellsize), 
                                   np.arange(ymin + cellsize/2, 
                                   ymax + cellsize/2, cellsize))
    xc = xx.flatten()
    yc = yy.flatten()
    print(xc,yc)
    pts = GeoSeries([Point(x, y) for x, y in zip(xc, yc)])
    in_map =  np.array([pts.within(geom) for geom in df.geometry]).sum(axis=0)
    pts = GeoSeries([val for pos,val in enumerate(pts) if in_map[pos]])
    pts.plot(markersize=3)
    pts.crs = "EPSG:32720"
    pts.to_file(outfile_point, driver="GeoJSON")
